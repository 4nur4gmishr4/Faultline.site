...
test.describe('FaultLine showcase', () => {
  ...
  test('donate page has free pitch and student presets', async ({ page }) => {
    ...
    // Desktop table cell (mobile card is display:none at default viewport)
    await expect(
      page.locator('table').getByText(/UPI \(VPA/i)
    ).toBeVisible()
    await expect(page.locator('table').getByText('0%').first()).toBeVisible()
    ...
    const upiCta = page.locator('a[href^="upi://pay"]').first()
    await expect(upiCta).toBeVisible()
    await expect(upiCta).toHaveAttribute('href', /upi:\/\/pay\?pa=anuragmishrasnag/)
  })
})
